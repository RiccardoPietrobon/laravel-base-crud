...
@section('page-name', 'Modifica Album' . $song->title)
...
@section('main-content')
    ...
    <form action="{{route('songs.update', $song)}}" method="POST" class="row g-3">

        @csrf{{-- attenzione --}}
        @method('PUT'){{-- direttiva per accettare PUT --}}

        <div class="col-4">
            <label for="title" class="form-label">Titolo</label>
            <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title') ?? $song->title }}">{{-- con OLD rimane quello che ho scritto in caso di errore --}}
        ...
        <div class="col-2">
            ...
            <input type="time" class="form-control @error('lenght') is-invalid @enderror" id="lenght" name="lenght" value="{{ old('lenght') ?? $song->lenght }}">
            @error('lenght')
                <div class="invalid-feedback">
                    {{$message}}
                </div>
